...
| 6 | **Done.** `main.c` is 232 lines: option parsing, the init order, the frame loop. `log.c`, `app.c` (SDL, window, GL, audio, the clock, screenshots), `splash.c` (the worker and the splash), `theatre.c` (power on and off, LEDs, fades, the drive), `input.c` (scancodes, capture, knobs, events), `selftest.c`. F15b fixed: under `--deterministic` the floppy LED follows the machine clock. That changed the LED patch in all three golden frames, which had captured a lagging real-time envelope; the references were re-blessed after confirming the difference was that patch alone (bbox 902..1029 × 738..797). Navigator and shell frames otherwise identical to the previous build; `--selftest` passes | M | golden + `--selftest` |
| 7 | **Done.** One `static struct` of state per module in `sound`, `corehost`, `install`, `ui`, `catalog`, `art` and `library` (F12), with initialisers as designated initialisers and the renames applied to code only, never to comments or strings. `library.c` builds every path through `path_join()`, which refuses a path that does not fit instead of truncating it (F15a); a game whose paths cannot be built is listed as not ready with the reason. Navigator frame identical, `--selftest` passes | S | build + `--selftest` |
