...
void input_capture(input_state *in,app *a,const dxm_layout *L,int on){
    ...
    } else ok=SDL_SetWindowMouseRect(a->win,NULL);
...
void input_init(input_state *in,app *a,const dxm_layout *L){
    in->knob_drag=-1; in->knob_y0=in->knob_v0=0.0f;
...
static void knob_turn(input_state *in,const app *a,gpu_knobs *k,float my){
    ...
    if(v<0.0f) v=0.0f;
    if(v>1.0f) v=1.0f;
    if(in->knob_drag==0) k->brightness=v; else k->contrast=0.4f+1.4f*v;
}

static void key_event(input_state *in,app *a,const dxm_layout *L,gpu_knobs *k,const SDL_Event *e){
...
input_result input_event(input_state *in,app *a,const dxm_layout *L,
                         gpu_knobs *k,const SDL_Event *e){
    switch(e->type){
    case SDL_EVENT_QUIT:
        return INPUT_QUIT;
    case SDL_EVENT_MOUSE_BUTTON_DOWN: {
        ...
        if(kn>=0 && e->button.button==SDL_BUTTON_LEFT){
            ...
            in->knob_drag=kn; in->knob_y0=my;
            in->knob_v0=(kn==0)?k->brightness:(k->contrast-0.4f)/1.4f;
        } else ui_mouse((int)mx,(int)my,1,0);
        break; }
    case SDL_EVENT_MOUSE_BUTTON_UP:
        if(in->knob_drag>=0) in->knob_drag=-1;
        else ui_mouse((int)(e->button.x*a->W/a->win_wf),(int)(e->button.y*a->H/a->win_hf),0,0);
        break;
    case SDL_EVENT_MOUSE_MOTION: {
        float mx=e->motion.x*a->W/a->win_wf, my=e->motion.y*a->H/a->win_hf;
        if(in->knob_drag>=0) knob_turn(in,a,k,my);
        else ui_mouse((int)mx,(int)my,(e->motion.state&SDL_BUTTON_LMASK)?1:0,1);
        break; }
    case SDL_EVENT_WINDOW_PIXEL_SIZE_CHANGED:
    case SDL_EVENT_WINDOW_DISPLAY_CHANGED: {
        int nw,nh; SDL_GetWindowSizeInPixels(a->win,&nw,&nh);
        if(nw>0 && nh>0 && (nw!=a->W || nh!=a->H)) return INPUT_RESIZED;
        break; }
...
void input_cursor(const input_state *in){
    if(in->captured && !ui_visible()) SDL_HideCursor();
    else SDL_ShowCursor();
}
