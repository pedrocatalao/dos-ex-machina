...
static struct {
    ...
    int n_games;
    /* Modules stay open once opened - see the header.  Parallel to `games`
     * by index, and reset with it. */
...
static SDL_EnumerationResult SDLCALL rm_entry(void *ud, const char *dirname, const char *fname) {
    int *fails = (int *)ud;
    char full[LIB_PATH];
    if (path_join(full, sizeof full, dirname, fname, NULL) != 0) {
        (*fails)++;
...
static int probe_found(const char *dir, const char *probe) {
    char p[LIB_PATH];
    if (path_join(p, sizeof p, dir, probe, NULL) != 0)
        return 0;
    if (exists(p))
        return 1;
    size_t base = strlen(dir);
    for (size_t i = base; p[i]; i++)
...
static void add_game(const char *id) {
    ...
    if (m.info->data_probe && !probe_found(g->data, m.info->data_probe))
        snprintf(g->note, sizeof g->note, "data missing (%s)", m.info->data_probe);
    else
        g->ready = 1;
    coreload_close(&m);
    lib.n_games++;
...
void lib_scan(void) {
    ...
    lib.n_games = 0;
    char dir[LIB_PATH];
    if (path_join(dir, sizeof dir, lib_root(), "games", NULL) != 0)
        return;
    if (!exists(dir)) {
...
const lib_game *lib_at(int i) {
    return (i >= 0 && i < lib.n_games) ? &lib.games[i] : NULL;
}
...
const dxm_module *lib_module(const lib_game *g) {
    ...
    if (i < 0 || i >= lib.n_games)
        return NULL;
    if (lib.mods[i].handle)
        return &lib.mods[i];
    char err[128];
...
void lib_touch_played(const lib_game *g) {
    char pl[LIB_PATH];
    if (path_join(pl, sizeof pl, g->dir, "played", NULL) != 0)
        return;
...
int lib_make_dir(const char *id, char *out, size_t outsz) {
    ...
    if (path_join(games_dir, sizeof games_dir, lib_root(), "games", NULL) != 0)
        return -1;
    if (!exists(games_dir) && !SDL_CreateDirectory(games_dir))
        return -1;
    if (path_join(out, outsz, lib_root(), "games", id, NULL) != 0)
        return -1;
    if (!exists(out) && !SDL_CreateDirectory(out))
        return -1;
    return 0;
}
