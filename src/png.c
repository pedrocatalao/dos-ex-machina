...
uint8_t *png_decode(const uint8_t *d, size_t n, int *ow, int *oh, char *err, size_t errsz) {
    ...
    while (p + 8 <= n) {
        ...
        if (!memcmp(type, "IHDR", 4)) {
            ...
            if (depth != 8 || interlace != 0 || (colour != 2 && colour != 6)) {
                snprintf(err, errsz, "unsupported PNG (need 8-bit RGB/RGBA, no interlace)");
                free(idat);
                return NULL;
            }
    ...
    zs.next_in = idat;
    zs.avail_in = (uInt)idat_n;
    zs.next_out = raw;
    zs.avail_out = (uInt)raw_n;
...
uint8_t *png_load(const char *path, int *w, int *h, char *err, size_t errsz) {
    ...
    if (!f) {
        snprintf(err, errsz, "cannot open image");
        return NULL;
    }
    fseek(f, 0, SEEK_END);
    long n = ftell(f);
    ...
    if (n <= 0 || n > 32L * 1024 * 1024) {
        fclose(f);
        snprintf(err, errsz, "image is not a sensible size");
        return NULL;
    }
