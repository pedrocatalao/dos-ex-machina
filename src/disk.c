...
static const char AUTOEXEC[] = "@ECHO OFF\r\n"
                               ...
                               "ECHO.\r\n";
static const char CONFIGSYS[] = "DEVICE=C:\\DOS\\HIMEM.SYS\r\n"
...
static const file *find(const char *dosname) {
    ...
    if (!SDL_strncasecmp(dosname, "C:\\", 3))
        dosname += 3;
    if (*dosname == '\\')
        dosname++;
    for (int i = 0; i < NFILES; i++)
        if (!SDL_strcasecmp(dosname, FILES[i].name))
            return &FILES[i];
    return NULL;
}
...
int disk_autoexec_echo(char (*lines)[80], int max) {
    char buf[4096];
    if (disk_read("AUTOEXEC.BAT", buf, sizeof buf) != 0)
        return 0;
    int n = 0;
    for (char *p = buf; *p && n < max;) {
        ...
        if (!SDL_strncasecmp(q, "ECHO", 4) && (q[4] == ' ' || q[4] == '.' || q[4] == 0)) {
            ...
            int skip = 0;
            if (*txt == '.')
                txt++; /* ECHO. is a blank line */
            else {
                while (*txt == ' ')
                    txt++;
                if (!SDL_strcasecmp(txt, "OFF") || !SDL_strcasecmp(txt, "ON"))
                    skip = 1;
            }
            if (!skip) {
                ...
                char *o = lines[n];
                int k = 0;
                for (const char *q2 = txt; *q2 && k < 79; q2++) {
                    if (*q2 == '^' && q2[1])
                        q2++;
                    o[k++] = *q2;
                }
