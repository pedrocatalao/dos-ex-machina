...
void boot_init(void){
    boot_step=0; t0=-1; next_boot=0;
    mem_counting=0; mem_shown=0;
    ax_n=disk_autoexec_echo(ax_lines,8); ax_i=0;
}
void boot_skip(void){ next_boot=0; }
...
static void mem_draw(long kb){
    ...
    int c=mem_col;
    for(const char *p=buf;*p&&c<DOS_COLS;p++) term_poke(mem_row,c++,*p);
    while(c<DOS_COLS && c<mem_col+12) term_poke(mem_row,c++,' ');
}
...
int boot_update(double t){
        ...
        if(t>=mem_next){
            ...
            if(mem_shown>=MEM_TOTAL_KB){
                ...
                { int c=mem_col+11; const char *ok=" OK";
                  for(const char *p=ok;*p&&c<DOS_COLS;p++) term_poke(mem_row,c++,*p); }
                mem_counting=0;
    ...
    if(t>=next_boot){
        if(BOOT[boot_step]){
            ...
            term_say(ln);
            if(strstr(ln,"Floppy Disk A")) machine.floppy_req=1.1;  /* BIOS seeks A: */
            if(strstr(ln,"Memory Test")){ /* start the live count here */
                mem_row=term_row(); mem_col=term_col();
        ...
        else if(ax_i<ax_n){ term_sayln(ax_lines[ax_i++]); next_boot=t+0.16; }
        else return 1;
    }
    return 0;
}
