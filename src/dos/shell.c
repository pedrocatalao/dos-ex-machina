...
static void oline(const char *s){
    if(!page_want){ term_sayln(s); return; }
    size_t l=strlen(s);
...
static void page_show(void){
    ...
    while(*page_pos && rows<23){
        ...
        memcpy(ln,page_pos,l); ln[l]=0; term_sayln(ln);
        page_pos=e?e+1:page_pos+l; rows++;
    }
    if(*page_pos) term_say("-- More --");
    else { page_on=0; shell_prompt(); }
}
...
static void page_key(int ch){
    if(ch==27||ch=='q'||ch=='Q'||ch==3){ page_on=0; term_put('\n'); shell_prompt(); return; }
    term_put('\n'); page_show();
}
...
void shell_init(void){ cmd_n=0; in_games=0; page_want=page_on=0; page_len=0; }
void shell_reset_line(void){ cmd_n=0; }
void shell_set_dir(int g){ in_games=g; }
...
static void cmd_dir(void){
    ...
    if(!in_games){
        ...
        { char ln[80];
          ...
          tot[o]=0;
          snprintf(ln,sizeof ln,"%9d file(s) %14s bytes",files,tot); term_sayln(ln);
          snprintf(ln,sizeof ln,"%9d dir(s)",dirs); term_sayln(ln); }
    } else {
...
static void run(char *s){
    ...
    if(!*s) return;
    if(!strcmp(s,"DIR")){      machine.floppy_req=0.7; cmd_dir(); }
    else if(!strcmp(s,"CLS")){ term_clear(); return; }
    else if(!strcmp(s,"HELP")) cmd_help();
    else if(!strcmp(s,"VER"))  term_sayln("DXM-DOS Version " DXM_VERSION " (C) 2026");
    else if(!strcmp(s,"TYPE")){
        static char buf[8192];
        if(!arg||!*arg) term_sayln("Required parameter missing");
        else {
            /* a path prefix is tolerated; only the root has files to show */
            int r=in_games?-1:disk_read(arg,buf,sizeof buf);
            if(r<0){ term_say("File not found - "); term_sayln(arg); }
            else if(r>0) term_sayln("This file cannot be displayed.");
            else {
                for(char *p=buf;*p;){
    ...
    else if(!strcmp(s,"EXIT")) { machine.state=DOS_OFF; return; }
    else if(!strcmp(s,"NC")){ nc_open_panel(in_games); in_games=1; return; }
    else if(!strcmp(s,"CD")||!strcmp(s,"CHDIR")){
        if(!arg||!*arg){ term_sayln(in_games?"C:\\GAMES":"C:\\"); }
        else if(!strcmp(arg,"\\")||!strcmp(arg,"/")) in_games=0;
        else if(!strcmp(arg,"..")){
            if(in_games) in_games=0;
            else term_sayln("Invalid directory");
        }
        ...
        else if(!g)        term_sayln("Bad command or file name");
        else if(!g->ready){ term_say("Cannot run "); term_say(g->title); term_sayln(":");
                            term_sayln(g->note[0]?g->note:"not ready"); }
        else { dos_launch(g->id); return; }
    }
}
...
void shell_key(int ch,int sc){
    ...
    if(ch=='\r'||ch=='\n'){
        ...
        if(machine.state==DOS_PROMPT && !nc_is_open() && !page_on) shell_prompt();
        return;
    }
    if(ch=='\b'){
        if(cmd_n){
            cmd_n--;
            if(term_col()>prompt_len) term_set_cursor(term_row(),term_col()-1);
            term_poke(term_row(),term_col(),' ');
        }
        return;
    }
    if(ch>=32 && ch<127 && cmd_n<(int)sizeof cmd-1){ cmd[cmd_n++]=(char)ch; term_put((char)ch); }
    (void)sc;
}
