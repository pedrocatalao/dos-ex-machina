...
const char *dos_launch_request(void){
    if(!machine.launch_pending) return NULL;
    machine.launch_pending=0; return machine.launch;
}

void dos_core_failed(void){
    term_put('\n');
    term_sayln("Cannot run that program.");
    shell_prompt(); machine.state=DOS_PROMPT; shell_reset_line();
}

void dos_core_exited(void){
    machine.state=DOS_PROMPT; shell_reset_line();
    if(nc_resume_after_game()) return;
    term_put('\n'); shell_prompt();
}

int dos_nc_open(void){ return nc_is_open(); }

void dos_key(int ch,int sc){
    if(machine.state!=DOS_PROMPT){ if(machine.state==DOS_BOOT) boot_skip(); return; }
    if(nc_is_open()){ nc_key(ch,sc); return; }
    shell_key(ch,sc);
}

/* The PC speaker POST beep: taken once, at power-on. */
int dos_take_beep(void){ int b=machine.beep_pending; machine.beep_pending=0; return b; }
double dos_take_floppy(void){ double f=machine.floppy_req; machine.floppy_req=0.0; return f; }

dos_state dos_update(double t){
    machine.now=t;
    if(nc_is_open()) nc_update(t);
    ...
    if(machine.state==DOS_RUNNING && machine.launch_at>0.0 && t>=machine.launch_at){
        machine.launch_pending=1; machine.launch_at=0.0;
    }

    if(machine.state==DOS_BOOT && boot_update(t)){
        machine.state=DOS_PROMPT; shell_prompt();
    }
...
const uint8_t *dos_render(void){
    ...
    term_render();
    if(nc_is_open()) nc_draw_art();
    if(machine.state==DOS_BOOT) boot_badge(machine.now);      /* POST only */
