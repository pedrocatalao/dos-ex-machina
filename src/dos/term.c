...
void term_put(char ch){
    if(ch=='\n'){ cur_c=0; if(++cur_r>=DOS_ROWS) term_scroll(); return; }
    if(cur_c>=DOS_COLS){ cur_c=0; if(++cur_r>=DOS_ROWS) term_scroll(); }
    att[cur_r][cur_c]=cur_att;
    scr[cur_r][cur_c++]=ch;
}
void term_say(const char *s){ while(*s) term_put(*s++); }
void term_sayln(const char *s){ term_say(s); term_put('\n'); }
void term_newline(void){ cur_c=0; if(++cur_r>=DOS_ROWS) term_scroll(); }
...
void term_fill_row(int r,int c,int n,int ch,uint8_t a){
    for(int k=0;k<n;k++) term_cell(r,c+k,ch,a);
}
void term_puts(int r,int c,const char *s,uint8_t a){
    for(int k=0;s[k];k++) term_cell(r,c+k,(unsigned char)s[k],a);
}
...
uint8_t term_attr_at(int r,int c){
    if(r<0||r>=DOS_ROWS||c<0||c>=DOS_COLS) return 0;
    return att[r][c];
}
int  term_row(void){ return cur_r; }
int  term_col(void){ return cur_c; }
void term_set_cursor(int r,int c){ cur_r=r; cur_c=c; }
uint8_t *term_fb(void){ return fb; }
