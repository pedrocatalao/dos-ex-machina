...
static void nc_rows_build(void) {
    ...
    for (int i = 0; i < lib_count() && nc_n < LIB_MAX; i++) {
        ...
        if (e->cat) {
            ...
            if (!e->version && e->cat->version[0])
                e->version = e->cat->version;
    ...
    for (int i = 0; i < cat_count() && nc_n < LIB_MAX; i++) {
        ...
        if (lib_find(c->id))
            continue; /* already on the machine */
        nc_entry *e = &nc_rows[nc_n++];
        memset(e, 0, sizeof *e);
        e->file = c->id;
        e->cmd = c->id;
        ...
        for (int k = 0; k < CAT_DESC; k++)
            e->desc[k] = c->desc[k][0] ? c->desc[k] : NULL;
    }
}
...
static void nc_flash_key(int slot) {
    nc_flash = slot;
    nc_flash_until = machine.now + 0.18;
}
...
static void nbox(int x, int y, int w, int h, const char *title, int active) {
    term_cell(y, x, BX_TL, A_PANEL);
    term_cell(y, x + w - 1, BX_TR, A_PANEL);
    term_cell(y + h - 1, x, BX_BL, A_PANEL);
    term_cell(y + h - 1, x + w - 1, BX_BR, A_PANEL);
    term_fill_row(y, x + 1, w - 2, BX_H, A_PANEL);
    term_fill_row(y + h - 1, x + 1, w - 2, BX_H, A_PANEL);
    for (int r = y + 1; r < y + h - 1; r++) {
        term_cell(r, x, BX_V, A_PANEL);
        term_cell(r, x + w - 1, BX_V, A_PANEL);
...
static void nrule(int x, int y, int w) { /* a divider across a panel */
    term_cell(y, x, BX_TE, A_PANEL);
    term_cell(y, x + w - 1, BX_TW, A_PANEL);
    term_fill_row(y, x + 1, w - 2, BX_H, A_PANEL);
}
...
static void nc_date(int64_t ns, char *out, size_t n) {
    SDL_DateTime dt;
    if (ns <= 0 || !SDL_TimeToDateTime(ns, &dt, true)) {
        out[0] = 0;
        return;
    }
...
static void nc_dialog(const char *title, const char *const *lines, int n, const char *keys,
                      int left) {
    int w = 50, h = n + 4, x = (DOS_COLS - w) / 2, y = 6;
    /* the shadow first, so the box paints over its inner edge */
    for (int r = y + 1; r < y + h + 1; r++)
        for (int c = x + 2; c < x + w + 2; c++)
            term_set_attr(r, c, A_SHDW);
    for (int r = y; r < y + h; r++)
        term_fill_row(r, x, w, ' ', A_DLG);
    term_cell(y, x, BX_TL, A_DLG);
    term_cell(y, x + w - 1, BX_TR, A_DLG);
    term_cell(y + h - 1, x, BX_BL, A_DLG);
    term_cell(y + h - 1, x + w - 1, BX_BR, A_DLG);
    term_fill_row(y, x + 1, w - 2, BX_H, A_DLG);
    term_fill_row(y + h - 1, x + 1, w - 2, BX_H, A_DLG);
    for (int r = y + 1; r < y + h - 1; r++) {
        term_cell(r, x, BX_V, A_DLG);
        term_cell(r, x + w - 1, BX_V, A_DLG);
    }
    if (title) {
        int tn = (int)strlen(title);
        term_puts(y, x + (w - tn - 2) / 2 + 1, title, A_DLG);
        term_cell(y, x + (w - tn - 2) / 2, ' ', A_DLG);
        term_cell(y, x + (w - tn - 2) / 2 + 1 + tn, ' ', A_DLG);
    }
    ...
    if (keys) {
        int kn = (int)strlen(keys);
        term_puts(y + h - 2, x + (w - kn) / 2, keys, A_DLGK);
    }
}
...
static void nc_draw(void) {
     ...
     * The headings sit in the first row; the list scrolls under them. */
    {
        ...
        for (int k = 0; k < rows; k++) {
            ...
            term_cell(y, NC_LX + NC_SEP1, BX_V, a);
            term_cell(y, NC_LX + NC_SEP2, BX_V, a);
             ...
             * holds it */
            {
                ...
                int u = 0;
                for (; e->file[u] && u < 15; u++)
                    up[u] = (char)toupper((unsigned char)e->file[u]);
                up[u] = 0;
            ...
            if (e->version) {
                ...
                if (e->update)
                    term_cell(y, NC_LX + NC_C_VER + (int)strlen(e->version) + 1, 0x18, a);
            }
            if (!e->installed) {
                int ax = NC_LX + NC_C_NAME + 1 + (int)strlen(e->file) + 1;
                if (ax < NC_LX + NC_SEP1)
                    term_cell(y, ax, 0x19, a);
                continue;
            }
            char d[12];
    ...
    if (nc_n == 0) {
        ...
        for (int r = NC_ART_T; r <= NC_ART_B; r++)
            term_fill_row(r, NC_RX + 1, NC_RW - 2, ' ', 0x00);
        ...
        enum { DL_MAX = 48 };
        static char dl[DL_MAX][NC_RW];
        static uint8_t da[DL_MAX];
        int nl = 0;
        {
            const int W = NC_RW - 4;
            char line[NC_RW];
            ...
            for (int k = 0; k < CAT_DESC; k++) {
                ...
                while (*w) {
                    ...
                    int len = (int)(end - w);
                    if (len > W)
                        len = W; /* a word longer than the
                                                      pane: cut it, do not
                                                      lose the line */
                    if (n && n + 1 + len > W) {
                        if (nl < DL_MAX) {
                            memcpy(dl[nl], line, (size_t)n + 1);
                            da[nl++] = A_TEXT;
                        }
                        n = 0;
                    }
        ...
        if (e->note && e->note[0]) {
            if (!e->installed && !e->available && nl < DL_MAX) {
                snprintf(dl[nl], NC_RW, "UNAVAILABLE");
                da[nl++] = A_HEAD;
            }
            if (nl < DL_MAX) {
                snprintf(dl[nl], NC_RW, "%s", e->note);
                da[nl++] = A_TEXT;
            }
        }
        {
            ...
            if (nc_dtop > 0)
                term_cell(NC_DESC_T, NC_RX + NC_RW - 1, 0x18, A_HEAD);
            if (nc_dtop + avail < nl)
                term_cell(NC_DESC_T + avail - 1, NC_RX + NC_RW - 1, 0x19, A_HEAD);
        }
        {
            inst_status is;
            install_poll(&is);
            ...
            if (busy) {
                ...
                char bar[34];
                int w = 30;
                int on = (int)(is.frac * w + 0.5);
                if (on > w)
                    on = w;
                if (on < 0)
                    on = 0;
                for (int k = 0; k < w; k++)
                    bar[k] = (char)(k < on ? 0xDB : 0xB0);
                bar[w] = 0;
                ...
                term_puts(NC_BOT - 3, NC_RX + 2, bar, A_NAME);
                {
                    char b[48];
                    if (is.total > 0)
                        snprintf(b, sizeof b, "%.0f%%  %.1f of %.1f MB", is.frac * 100.0,
                                 is.got / 1048576.0, is.total / 1048576.0);
                    else
                        snprintf(b, sizeof b, "%.1f MB", is.got / 1048576.0);
        ...
        nrule(NC_RX, NC_BOT - 1, NC_RW);
        {
            char foot[64];
            if (e->year)
                snprintf(foot, sizeof foot, "%s, %d", e->by, e->year);
            else
                snprintf(foot, sizeof foot, "%s", e->by);
    ...
    static const char *KEYS[10] = {"Help  ", "Delete", "Reset ", "Update", "      ",
                                   "      ", "      ", "      ", "      ", "Quit  "};
    for (int k = 0; k < 10; k++) {
        char num[3];
        snprintf(num, sizeof num, "%2d", k + 1);
...
void nc_draw_art(void) {
    ...
    if (aw > bw || ah > bh) {
        ...
        int step = 1;
        while ((aw / step > bw || ah / step > bh) && step < 32)
            step++;
...
void nc_key(int ch, int sc) {
    ...
    if (nc_dlg) {
        ...
        if (yes && was == DLG_DELETE) {
            ...
            nc_rows_build();
            if (nc_sel >= nc_n)
                nc_sel = nc_n ? nc_n - 1 : 0;
            if (left) {
                snprintf(nc_note[0], sizeof nc_note[0], "Some files could not be removed.");
        ...
        if (yes && was == DLG_UPDATE) {
            ...
            if (g)
                lib_unload(g); /* the download overwrites game.dxm */
            install_clear();
        ...
        if (yes && was == DLG_RESET) {
            ...
            if (r == 0) {
                ...
                snprintf(nc_note[1], sizeof nc_note[1], "%.63s",
                         r > 0 ? "no archive, and nothing to fetch" : err);
                nc_dlg = DLG_NOTE;
            }
            lib_scan();
            nc_rows_build();
    ...
    if (sc == SC_F2 || sc == SC_F3 || sc == SC_F4) {
        ...
        char l1[64];
        if (!e)
            nc_say("There is nothing here to act on.", NULL);
        else if (is.state == INST_RUNNING)
            nc_say("A download is running.", "Wait for it to finish.");
        else if (!e->installed) {
            ...
            nc_dlg = (sc == SC_F2) ? DLG_DELETE : (sc == SC_F3) ? DLG_RESET : DLG_UPDATE;
        nc_draw();
        return;
    }
    ...
    if (sc == DXM_SC_ESC || ch == 27 || sc == DXM_SC_F10) {
        nc_open = 0;
        shell_set_dir(nc_from_games); /* back where it was opened */
        term_clear();
        shell_prompt();
        return;
    }
    ...
    if (nc_focus == 1) {
        ...
        if (sc == DXM_SC_DOWN)
            nc_dtop++;
        else if (sc == DXM_SC_UP) {
            if (nc_dtop > 0)
                nc_dtop--;
        } else if (sc == DXM_SC_RIGHT)
            nc_dtop += avail;
        else if (sc == DXM_SC_LEFT) {
            nc_dtop -= avail;
            if (nc_dtop < 0)
                nc_dtop = 0;
        }
    } else if (sc == DXM_SC_DOWN) {
        if (nc_sel + 1 < nc_n) {
            nc_sel++;
            nc_dtop = 0;
        }
        ...
        if (e->installed) {
            ...
            shell_prompt();
            {
                ...
                int k = 0;
                for (; e->cmd[k] && k < 15; k++)
                    up[k] = (char)toupper((unsigned char)e->cmd[k]);
                up[k] = 0;
                term_sayln(up);
            }
            dos_launch(e->cmd);
...
void nc_update(double t) {
    ...
    if (is.state == INST_RUNNING || is.state != was)
        nc_draw();
    was = is.state;
}
