...
static void read_folder(const char *dos_dir) {
    ...
    char up[WHERE_PATH];
    if (dos_up(dos_dir, up, sizeof up)) {
        dos_trim_sep(up);
        const char *parent = dos_leaf(up);
        for (int i = 0; i < categories(); i++)
            if (!SDL_strcasecmp(parent, CATEGORY_OPT[i]))
                E.category = i;
    }

    const char *name = dos_leaf(dos_dir);
    ...
    for (int i = 0; i < n; i++) {
        ...
        browse_split(found[i].rel, dir, sizeof dir, base, sizeof base);
    ...
    if (!E.work.setup[0])
        for (int i = 0; i < n; i++) {
            ...
            browse_split(found[i].rel, sdir, sizeof sdir, sbase, sizeof sbase);
...
static void add_chose_zip(void) {
    cat_title t;
    memset(&t, 0, sizeof t);
    guess_id(dos_leaf(E.add_zip), t.id, sizeof t.id);
    if (!t.id[0])
...
/* A folder on a drive.  Which question it answers depends on what the
 * browser was opened for: where a title already is, or where one is going. */
void browse_took_folder(const char *dos_dir, const char *chosen, browse_for what) {
    if (what == BROWSE_DEST) {
        ...
        if (E.dest_point)
            snprintf(E.dest, sizeof E.dest, "%s", dos_dir);
        else
            snprintf(E.dest, sizeof E.dest, "%s\\%s", dos_dir, E.dest_id);
        E.fault[0] = 0;
        E.panel = PANEL_DEST;
        return;
    }
    if (chosen) { /* a program was picked inside it, which settles `run` too */
        snprintf(E.work.run, sizeof E.work.run, "%s", chosen);
        dos_upper(E.work.run);
    }
    snprintf(E.add_dir, sizeof E.add_dir, "%s", dos_dir);
    add_chose_dir();
}

/* One of the programs under a folder, chosen from the list of them. */
void browse_took_program(const char *dos_dir, const char *name, browse_for what) {
    if (what == BROWSE_SETUP)
        snprintf(E.work.setup, sizeof E.work.setup, "%s", name);
    else {
        snprintf(E.work.run, sizeof E.work.run, "%s", name);
        /* where it sits is where it runs, and a setup found beside the old
         * one is not necessarily beside this one */
        snprintf(E.add_dir, sizeof E.add_dir, "%s", dos_dir);
        E.work.setup[0] = 0;
    }
    dos_upper(E.work.run);
    dos_upper(E.work.setup);
    E.fault[0] = 0;
    E.panel = PANEL_TITLE;
    build_title_form();
}
...
static void add_take(int way) {
    ...
    if (way == ADD_FROM_ZIP) {
        ...
        E.add_step = ADD_HERE;
        browse_drive(root, BROWSE_FOLDER);
    }
}
...
static void draw_dest(void) {
    ...
    gui_fit(E.dest, DW - 64, 1, line, sizeof line);
