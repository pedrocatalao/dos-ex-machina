...
static int on_progress(void *p, curl_off_t dt, curl_off_t dn, curl_off_t ut, curl_off_t un) {
    (void)ut;
    (void)un;
    prog *g = p;
...
static CURL *setup(const char *url, prog *g, char *err, size_t errsz) {
    ...
    if (!c) {
        snprintf(err, errsz, "curl unavailable");
        return NULL;
    }
...
int net_get_file(const char *url, const char *path, net_progress cb, void *ud, volatile int *cancel,
    ...
    if (r != CURLE_OK) {
        if (r == CURLE_ABORTED_BY_CALLBACK)
            ...
            snprintf(err, errsz, "HTTP %ld", code);
        else
            snprintf(err, errsz, "%s", curl_easy_strerror(r));
        remove(tmp);
        return -1;
    }
    remove(path);
...
static size_t to_mem(void *p, size_t sz, size_t n, void *ud) {
    ...
    if (m->n + add + 1 > m->cap) {
        ...
        while (cap < m->n + add + 1)
            cap *= 2;
        if (cap > NET_MAX_MEM)
            return 0; /* refuse to grow forever */
...
int net_get_mem(const char *url, char **out, size_t *outn, char *err, size_t errsz) {
    net_init();
    membuf m = {NULL, 0, 0};
