...
void snd_floppy(double seconds) {
    if (seconds > snd.fdd_left) {
        if (snd.fdd_left <= 0.0)
            snd.fdd_pos = 0.0; /* start of a fresh access */
        snd.fdd_left = seconds;
    }
}
float snd_floppy_level(void) {
    return snd.fdd_env;
}
...
void snd_mix(int16_t *out, int nframes) {
    ...
    for (int i = 0; i < nframes; i++) {
        ...
        if (snd.dg_t >= 0.0) {
            ...
            float buzz = (float)(sin(snd.dg_t * 6.28318530718 * 50.0) +
                                 0.35 * sin(snd.dg_t * 6.28318530718 * 100.0));
        ...
        if (snd.fdd_left > 0.0 || snd.fdd_env > 0.001f) {
            ...
            snd.fdd_pos += step;
            if (snd.fdd_pos >= FDD_PCM_LEN)
                snd.fdd_pos -= FDD_PCM_LEN;
        ...
        if (snd.spk_left > 0) {
            ...
            if (done < snd.rate / 200)
                env = done / (float)(snd.rate / 200);
            if (snd.spk_left < snd.rate / 73)
                env = fminf(env, snd.spk_left / (float)(snd.rate / 73));
