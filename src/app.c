...
Uint64 app_now_ns(void){ return g_fixed_step ? g_vclock : SDL_GetTicksNS(); }
void   app_fixed_step(void){ g_vclock=SDL_GetTicksNS(); g_fixed_step=1; }
void   app_frame_done(void){ if(g_fixed_step) g_vclock+=1000000000ull/60; }
...
void app_measure(app *a){
    ...
    int ww,wh; SDL_GetWindowSize(a->win,&ww,&wh);
    a->win_wf=(float)(ww>0?ww:a->W); a->win_hf=(float)(wh>0?wh:a->H);
}

int app_init(app *a,const app_options *o){
    memset(a,0,sizeof *a);
    a->windowed=o->windowed; a->deterministic=o->deterministic;
    if(o->audio_dump) g_audio_dump=fopen(o->audio_dump,"wb");
    ...
    if(!o->deterministic) wflags|=SDL_WINDOW_HIGH_PIXEL_DENSITY;
    if(!o->windowed) wflags|=SDL_WINDOW_FULLSCREEN;
    ...
    { /* SDL_CreateSurfaceFrom wants writable pixels and the icon is
      ...
      if(icpx){
          ...
          if(ic){ SDL_SetWindowIcon(a->win,ic); SDL_DestroySurface(ic); }
          free(icpx);
      } }
    ...
    app_measure(a);
    dxm_log("window %dx%d px (%.0fx%.0f units), %s",a->W,a->H,(double)a->win_wf,(double)a->win_hf,
    ...
    { uint8_t *spl=dxm_splash_rgba();
      if(spl){ gpu_set_splash(a->gpu,spl,DXM_SPLASH_W,DXM_SPLASH_HT); free(spl); } }
    ...
    if(a->audio) SDL_ResumeAudioStreamDevice(a->audio);
    if(a->audio) dxm_log("audio stream open");
...
void app_screenshot(const app *a,const char *path){
    int rw,rh; uint8_t *px=gpu_readback(a->gpu,&rw,&rh);
    if(px){
        write_bmp(path,px,rw,rh); free(px);
...
void app_shutdown(app *a){
    corehost_stop();
    if(g_audio_dump){ fclose(g_audio_dump); g_audio_dump=NULL; }
    log_close();
    SDL_Quit();
    (void)a;
}
