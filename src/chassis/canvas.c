                ...
                if (gl[j] & (0x80 >> i))
                    for (int sy = 0; sy < (int)sc; sy++)
                        for (int sx = 0; sx < (int)sc; sx++) {
                            px_blend(c, (int)(x + (n * 8 + i) * sc + sx),
                                     (int)(y + j * sc + sy) + 1, 255, 255, 255, 0.13f);
                            px_set(c, (int)(x + (n * 8 + i) * sc + sx), (int)(y + j * sc + sy), r,
                                   g, b);
                        }
    }
}
            ...
            for (int i = 0; i < (int)ceilf(gw); i++) {
                ...
                for (int sy = 0; sy < 4; sy++)
                    for (int sx = 0; sx < 4; sx++) {
                        ...
                        if (ui >= 0 && ui < 8 && vi >= 0 && vi < 8 && (gl[vi] & (0x80 >> ui)))
                            on++;
                    }
                if (!on)
                    continue;
                float a = (float)on / 16.0f;
                px_blend(c, (int)(gx + i), (int)(y + j) + 1, 255, 255, 255,
                         0.13f * a); /* the lit lower edge */
