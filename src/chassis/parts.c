...
void floppy_drive(canvas *c,float x,float y,float w,float h,float led_out[4]){
    ...
    led_out[0]=x+16.5f*mm-2.3f*mm; led_out[1]=y+18.6f*mm-1.1f*mm;
    led_out[2]=4.6f*mm;            led_out[3]=2.2f*mm;
}
...
void power_button(canvas *c,float px0,float pw,float mid,float mm,float band_h,dxm_layout *L){
        ...
        { float lr=1.35f*mm;
          ...
          L->pwr_led[0]=lcx-lr; L->pwr_led[1]=lcy-lr;
          L->pwr_led[2]=lr*2.0f; L->pwr_led[3]=lr*2.0f;
          L->pwr_shelf=cap_lo; }
}
