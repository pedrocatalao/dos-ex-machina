...
static void oops(char *err, size_t n, const char *fmt, ...) __attribute__((format(printf, 3, 4)));
...
int coreload_open(dxm_module *m, const char *path, char *err, size_t errsz) {
    ...
    if (m->info->abi != DXM_ABI) {
        char buf[96];
        snprintf(
            buf, sizeof buf, "module speaks ABI %d, this DXM speaks %d - %s", m->info->abi, DXM_ABI,
            m->info->abi > DXM_ABI ? "the machine needs updating" : "the game needs rebuilding");
