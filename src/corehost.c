...
static void h_present(const dxm_frame *f){
    ...
    SDL_LockMutex(mu); front=back; back=1-back; SDL_UnlockMutex(mu);
 ...
 * library of its own. */
static SDL_Mutex *game_mu;
static void   h_lock(void)  { if(game_mu) SDL_LockMutex(game_mu); }
static void   h_unlock(void){ if(game_mu) SDL_UnlockMutex(game_mu); }
...
int corehost_start(const dxm_core_info *info,const char *data_dir){
    ...
    th = SDL_CreateThread(thread_main,"core",NULL);
    if(!th){ running=0; return -1; }
    return 0;
}
void corehost_stop(void){
    if(!running && front<0) return;
    quit_req=1;
    for(int i=0;i<200 && running;i++) SDL_Delay(10);   /* ~2s (PORTING §3.6) */
    SDL_WaitThread(th,NULL); th=NULL;
    running=0; front=-1; quit_req=0;
}
int corehost_running(void){ return running; }
const uint8_t *corehost_frame(int *w,int *h,int *lines){
    if(!mu) return NULL;
    SDL_LockMutex(mu); int f=front; SDL_UnlockMutex(mu);
    if(f<0) return NULL;
