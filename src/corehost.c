...
static struct {
    ...
    /* No core is linked in: a game is always a module opened at run time, so *these start NULL and corehost_start refuses until one is chosen. */
    dxm_core_main_fn f_main;
    dxm_core_audio_fn f_audio;
    /* The core's one lock lives here, not in the core: the shell owns the *thread, and a core shipped as a loadable module must not link a threading *library of its own. */
    SDL_Mutex *game_mu;
} core = { .front=-1 };
...
static double now_s(void){ return (double)SDL_GetTicksNS() * 1e-9; }
static void h_present(const dxm_frame *f){
    const dxm_mode *m=f->mode;
    int n=m->w*m->h;
    if((size_t)n*3 > sizeof core.rgb[0]) return;
    ...
    core.fw[core.back]=m->w; core.fh[core.back]=m->h; core.flines[core.back]=m->crt_lines;
    SDL_LockMutex(core.mu); core.front=core.back; core.back=1-core.back; SDL_UnlockMutex(core.mu);
...
static int    h_key_down(int sc){ return sc>=0&&sc<NKEYS ? core.keys[sc] : 0; }
static int    h_getch(void){
    if(core.chq_r==core.chq_w) return 0;
    int v=core.chq[core.chq_r]; core.chq_r=(core.chq_r+1)%64; return v;
}
static int    h_should_quit(void){ return core.quit_req; }
static double h_now(void){ return now_s(); }
static void   h_sleep(int ms){ SDL_Delay((Uint32)ms); }
static void   h_log(const char *m){ fprintf(stderr,"[core] %s\n",m); }

static void   h_lock(void)  { if(core.game_mu) SDL_LockMutex(core.game_mu); }
static void   h_unlock(void){ if(core.game_mu) SDL_UnlockMutex(core.game_mu); }
...
int corehost_start(const dxm_core_info *info,const char *data_dir){
    ...
    cur_info=info; snprintf(core.cur_data,sizeof core.cur_data,"%s",data_dir);
    ...
    else snprintf(core.cur_pref,sizeof core.cur_pref,"%s",core.cur_data);
    HOST.data_dir=core.cur_data; HOST.pref_dir=core.cur_pref;
    for(size_t i=0;i<sizeof core.keys/sizeof core.keys[0];i++) core.keys[i]=0;
    core.chq_r=core.chq_w=0;
    core.front=-1; core.back=0; core.quit_req=0; core.running=1;
    if(!core.mu)      core.mu      = SDL_CreateMutex();
    if(!core.game_mu) core.game_mu = SDL_CreateMutex();
    core.th = SDL_CreateThread(thread_main,"core",NULL);
    if(!core.th){ core.running=0; return -1; }
    return 0;
}
void corehost_stop(void){
    if(!core.running && core.front<0) return;
    core.quit_req=1;
    for(int i=0;i<200 && core.running;i++) SDL_Delay(10);   /* ~2s (PORTING §3.6) */
    SDL_WaitThread(core.th,NULL); core.th=NULL;
    core.running=0; core.front=-1; core.quit_req=0;
}
int corehost_running(void){ return core.running; }
const uint8_t *corehost_frame(int *w,int *h,int *lines){
    if(!core.mu) return NULL;
    SDL_LockMutex(core.mu); int f=core.front; SDL_UnlockMutex(core.mu);
    if(f<0) return NULL;
    *w=core.fw[f]; *h=core.fh[f]; *lines=core.flines[f];
    return core.rgb[f];
}
void corehost_push_key(int sc,int down,int ch){
    if(sc>=0&&sc<NKEYS) core.keys[sc]=(unsigned char)down;
    if(down&&ch){ int n=(core.chq_w+1)%64; if(n!=core.chq_r){ core.chq[core.chq_w]=ch; core.chq_w=n; } }
}
void corehost_audio(int16_t *out,int nframes){
    ...
    if(ok && core.f_audio) core.f_audio(out,nframes);
