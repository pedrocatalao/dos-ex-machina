...
int plat_pump(void) {
    return !H->should_quit();
}

unsigned plat_keys(void) {
    unsigned m = 0;
    if (H->key_down(DXM_SC_LEFT))
        m |= K_LEFT;
    if (H->key_down(DXM_SC_RIGHT))
        m |= K_RIGHT;
    if (H->key_down(DXM_SC_UP))
        m |= K_UP;
    if (H->key_down(DXM_SC_DOWN))
        m |= K_DOWN;
    if (H->key_down(DXM_SC_SPACE))
        m |= K_SPACE;
    if (H->key_down(DXM_SC_ESC))
        m |= K_ESC;
    if (H->key_down(DXM_SC_ENTER))
        m |= K_RET;
    return m;
}
int plat_getch(void) {
    int c = H->getch();
    if (c & 0x100)
        return 0; /* extended: not plain ASCII */
    return c;
}
